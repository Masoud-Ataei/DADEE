...
class estimate_Fx_NN_Anchor_Ensemble_DEUP():
    # global coef_loss
    def __init__(self, n_net = 5, n_in = 2, n_out = 12, 
                 n_hidden = (4,12), lr = 0.01,        
                 activ = "tanh", reg = 0.0, coef_loss = 1.0, RMS_zero = True, specnorm = False, prior = [0.0, 1.0]):        
        """
        ...
        for i in range(n_net):
            nn, theta = self.def_model(n_in = n_in, n_out = n_out,
                                              activ = activ ,lr = lr,
                                              n_layer = n_layer, n_node = n_node, prior = prior, specnorm = specnorm, loss_func = self.custom_loss)
            nns.append(nn)
    ...
    def load_model(self, path='dump.sav'):
        ...
        for i,w in enumerate(weights_mu):
            self.models_mu[i].set_weights(w)
    ...
    def def_model(self, n_in = 2, n_out = 12, n_layer = 4, n_node = 12, 
                  activ = "tanh", lr = 0.01, opt = 'Adam', prior = [0.0, 1.0], specnorm = False, loss_func = None):
    ...
    def fit_models(self, data_set, epochs=10, batch_size=100, epochs_var = 10, lr=0.001, delta=0.01, opt='Adam', const=0.0, verbose = True, val_data = None, preloss = None):
        self.N = len(data_set)
        self.set_lr(lr)
        # xin = tf.convert_to_tensor(list(data_set))[:,0,:]
        # yin = tf.convert_to_tensor(list(data_set))[:,1,:]
        xin = tf.convert_to_tensor([l[0] for l in list(data_set)])
        yin = tf.convert_to_tensor([l[1] for l in list(data_set)])
        ...
        for i, model_mu in enumerate(self.models_mu):
            self.model_index = i            
            self.hmu.append(model_mu .fit(xin, yin, validation_data = val_data, batch_size=batch_size , epochs=epochs, verbose=verbose).history)
        
        mu  = np.array([model_mu (xin) for model_mu in self.models_mu])
        Mmu = np.mean(mu, axis = 0)            
        Vmu = np.var (mu, axis = 0)
        ein = (yin - Mmu) ** 2 - Vmu
        ein = tf.maximum(ein, ein * 0.0)
        if val_data is not None:
            yhatval  = np.array([model_mu (xval) for model_mu in self.models_mu])
            Mmuval = np.mean(yhatval, axis = 0)            
            Vmuval = np.var (yhatval, axis = 0)
            einval = (yval - Mmuval) ** 2 - Vmuval
            einval = tf.maximum(einval, einval * 0.0)
            val_data = (xval, einval)
        loss_var   = self.model_var.fit(xin, ein, validation_data = val_data, batch_size=batch_size , epochs=epochs_var, verbose=verbose).history

        loss = {}
        for k in self.hmu[0].keys():
            loss[k] = list(np.array([l[k] for l in self.hmu ]).mean(axis=0))
